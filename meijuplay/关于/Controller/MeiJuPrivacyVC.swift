...
class MeiJuPrivacyVC: MWBaseViewController {

    // MARK: - properteis
    fileprivate var displayTextView: UITextView?
    ...
    fileprivate func setupSubviews() {
        if displayTextView == nil {
            displayTextView = UITextView(frame: self.view.bounds)
        }
        self.view.addSubview(displayTextView!)
        displayTextView?.isEditable = false
        displayTextView?.font = UIFont.MWCustomFont.descFont
        displayTextView?.textColor = UIColor.MWCustomColor.primaryText
...
用户服务协议和隐私政策
《用户服务协议和隐私政策》（以下简称“协议”）及其条款，系您下载、安装及使用“不止美剧”软件（以下简称“本软件”）所订立的、描述您与本软件之间权利义务的协议。
...
我们在本软件 App 上提供以下方式帮助您删除您在本软件上发布的信息。  您可通过【社区→我的帖子】，删除您在社区中展示的信息；  您可通过【我的 → 关注】，取消您的关注记录；  您可通过【我的 →设置】，取消运势提醒；  您可通过【我的 →设置】，取消私信开关。
